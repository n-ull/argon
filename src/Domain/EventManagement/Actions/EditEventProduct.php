...
class EditEventProduct
{
    ...
    public function handle(Event $event, Product $product, array $data): Product
    {
        ...
        $product->update([
            ...
            'ticket_type' => $data['ticket_type'] ?? $product->ticket_type ?? 'dynamic',
        ...
        if (!empty($data['prices'])) {
            ...
            // 2. Delete prices that are not in the request
            $product->product_prices()
                ->whereNotIn('id', $requestPriceIds)
                ->delete();
