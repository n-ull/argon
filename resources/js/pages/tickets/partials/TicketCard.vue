...
<template>
    <div class="group relative flex flex-col overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm transition-all duration-300 hover:shadow-md dark:border-zinc-800 dark:bg-zinc-900"
        :class="{ 'opacity-75 grayscale-[0.5]': isUsed }">
        <!-- Card Header with Image/Icon -->
        <div class="relative h-32 w-full overflow-hidden bg-zinc-100 dark:bg-zinc-800">
            ...
            <div v-else class="flex h-full w-full items-center justify-center">
                <TicketIcon class="h-12 w-12 text-zinc-300 dark:text-zinc-700" />
            </div>

            <!-- Status Badge -->
            <div class="absolute right-3 top-3">
                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium"
                    :class="isUsed ? 'bg-zinc-100 text-zinc-800 dark:bg-zinc-800 dark:text-zinc-300' : 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400'">
                    {{ isUsed ? 'Used' : 'Active' }}
                </span>
            </div>
        </div>
        ...
        <div class="flex flex-1 flex-col p-4">
            <h3 class="line-clamp-1 text-lg font-semibold text-zinc-900 dark:text-zinc-100">
            ...
            <div class="mt-6 flex items-center justify-between border-t border-zinc-100 pt-4 dark:border-zinc-800">
                ...
                <Link :href="`/tickets/${ticket.token}`"
                    class="inline-flex items-center rounded-lg bg-zinc-900 px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-zinc-800 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-100">
                    View details
